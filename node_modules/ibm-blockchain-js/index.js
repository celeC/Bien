...
//		console.log(options.network.users);
	...
	if(options.network.users && options.network.users.length > 0){
		...
		async.each(arr, function(i, a_cb) {
			if(options.network.users[i]){											//make sure we still have a enrollId for this network
				...
				console.log("JACEY REGISTER ENROLLID");
...
ibc.prototype.load_chaincode = function(options, cb) {
	...
	function parse_for_invoke(name, str){
		if(str == null) logger.error('! [ibc-js] fs readfile Error');
			...
			// Step 2a.
			var go_func_regex = /func\s+\(\w+\s+\*BienChaincode\)\s+(\w+)/g;			//find chaincode's go lang functions
			var result;
...
function read(args, enrollId, cb){
	...
	options.failure = function(statusCode, e){
		logger.error('[ibc-js] (Read) - failure:',statusCode, e);
...
function deploy(func, args, deploy_options, enrollId, cb){
	...
	if(ibc.chaincode.details.version.indexOf('hyperledger/fabric/core/chaincode/shim') >= 0){	//hyperledger body format
		...
		console.log("[JACEY]enrollID:   "+enrollId);
		body = 	{
					jsonrpc: '2.0',
					method: 'deploy',
					params: {
	...
	options.success = function(statusCode, data){
		...
		logger.log('[ibc-js] ibc.chaincode.details.deployed_name:'+ibc.chaincode.details.deployed_name);
	};
